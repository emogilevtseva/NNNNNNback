...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'tableanagram.xlsx', 'path': 'tableanagram.xlsx'},
...
async function experimentInit() {
  ...
  instr = new visual.TextStim({
    win: psychoJS.window,
    name: 'instr',
    text: 'В рамках этого эксперимента Вам предстоит решить ряд анаграмм, то есть составить существующее слово, содержащее в себе все буквы из изначальной последовательности. Время на решение и ответ будет ограничено 5 минутами. После каждой задачи Вам будет необходимо оценить чувство уверенность в ответе по шкале от 1 до 100. Чтобы перейти от решения анаграммы к оценке уверенности в ответе, нажмите ПРОБЕЛ\n\nЧтобы перейти к прохождению эксперимента, нажмите ПРОБЕЛ',
    font: 'Arial',
    units: undefined, 
  ...
  // Initialize components for Routine "angrroutine1"
  angrroutine1Clock = new util.Clock();
  angr1 = new visual.TextStim({
    win: psychoJS.window,
    name: 'angr1',
    text: '',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0.2], draggable: false, height: 0.25,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
    color: new util.Color('white'),  opacity: undefined,
    depth: 0.0 
  });
  
  textbox1a = new visual.TextBox({
    win: psychoJS.window,
    name: 'textbox1a',
    text: '',
    placeholder: 'Ответ',
  ...
  key_resp1_1 = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  
  // Initialize components for Routine "question1"
  question1Clock = new util.Clock();
  text_qu1 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_qu1',
    text: 'Оцените чувство уверенности в ответе от 1 до 100\n\n\nПосле, чтобы перейти к следующему заданию, нажмите ПРОБЕЛ',
    font: 'Arial',
    units: undefined, 
  ...
  textbox_qu1 = new visual.TextBox({
    win: psychoJS.window,
    name: 'textbox_qu1',
    text: '',
    placeholder: 'Ответ',
  ...
  key_resp_qu1 = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
...
function instructionRoutineBegin(snapshot) {
  return async function () {
    ...
    instructionComponents.push(key_instr);
...
function instructionRoutineEachFrame() {
  return async function () {
    ...
    // *key_instr* updates
    if (t >= 5 && key_instr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials.forEach(function() {
      ...
      trialsLoopScheduler.add(angrroutine1RoutineBegin(snapshot));
      trialsLoopScheduler.add(angrroutine1RoutineEachFrame());
      trialsLoopScheduler.add(angrroutine1RoutineEnd(snapshot));
      trialsLoopScheduler.add(question1RoutineBegin(snapshot));
      trialsLoopScheduler.add(question1RoutineEachFrame());
      trialsLoopScheduler.add(question1RoutineEnd(snapshot));
...
var angrroutine1MaxDurationReached;
var _key_resp1_1_allKeys;
var angrroutine1MaxDuration;
var angrroutine1Components;
function angrroutine1RoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'angrroutine1' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    angrroutine1Clock.reset(routineTimer.getTime());
    routineTimer.add(303.000000);
    angrroutine1MaxDurationReached = false;
    // update component parameters for each repeat
    angr1.setText(anagram);
    textbox1a.setText('');
    textbox1a.refresh();
    key_resp1_1.keys = undefined;
    key_resp1_1.rt = undefined;
    _key_resp1_1_allKeys = [];
    psychoJS.experiment.addData('angrroutine1.started', globalClock.getTime());
    angrroutine1MaxDuration = null
    // keep track of which components have finished
    angrroutine1Components = [];
    angrroutine1Components.push(angr1);
    angrroutine1Components.push(textbox1a);
    angrroutine1Components.push(key_resp1_1);
    
    angrroutine1Components.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
var frameRemains;
function angrroutine1RoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'angrroutine1' ---
    // get current time
    t = angrroutine1Clock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    
    // *angr1* updates
    if (t >= 0.5 && angr1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      angr1.tStart = t;  // (not accounting for frame time here)
      angr1.frameNStart = frameN;  // exact frame index
      
      angr1.setAutoDraw(true);
    }
    
    
    // if angr1 is active this frame...
    if (angr1.status === PsychoJS.Status.STARTED) {
    }
    
    frameRemains = 0.5 + 300.5 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (angr1.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      // keep track of stop time/frame for later
      angr1.tStop = t;  // not accounting for scr refresh
      angr1.frameNStop = frameN;  // exact frame index
      // update status
      angr1.status = PsychoJS.Status.FINISHED;
      angr1.setAutoDraw(false);
    }
    
    
    // *textbox1a* updates
    if (t >= 0.5 && textbox1a.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      textbox1a.tStart = t;  // (not accounting for frame time here)
      textbox1a.frameNStart = frameN;  // exact frame index
      
      textbox1a.setAutoDraw(true);
    }
    
    
    // if textbox1a is active this frame...
    if (textbox1a.status === PsychoJS.Status.STARTED) {
    }
    
    frameRemains = 0.5 + 300 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (textbox1a.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      // keep track of stop time/frame for later
      textbox1a.tStop = t;  // not accounting for scr refresh
      textbox1a.frameNStop = frameN;  // exact frame index
      // update status
      textbox1a.status = PsychoJS.Status.FINISHED;
      textbox1a.setAutoDraw(false);
    }
    
    
    // *key_resp1_1* updates
    if (t >= 3 && key_resp1_1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      key_resp1_1.tStart = t;  // (not accounting for frame time here)
      key_resp1_1.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { key_resp1_1.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { key_resp1_1.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { key_resp1_1.clearEvents(); });
    }
    frameRemains = 3 + 300 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
    if (key_resp1_1.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      // keep track of stop time/frame for later
      key_resp1_1.tStop = t;  // not accounting for scr refresh
      key_resp1_1.frameNStop = frameN;  // exact frame index
      // update status
      key_resp1_1.status = PsychoJS.Status.FINISHED;
      frameRemains = 3 + 300 - psychoJS.window.monitorFramePeriod * 0.75;// most of one frame period left
      if (key_resp1_1.status === PsychoJS.Status.STARTED && t >= frameRemains) {
        // keep track of stop time/frame for later
        key_resp1_1.tStop = t;  // not accounting for scr refresh
      ...
      // if key_resp1_1 is active this frame...
      if (key_resp1_1.status === PsychoJS.Status.STARTED) {
        let theseKeys = key_resp1_1.getKeys({
          keyList: typeof 'space' === 'string' ? ['space'] : 'space', 
          waitRelease: false
        });
        _key_resp1_1_allKeys = _key_resp1_1_allKeys.concat(theseKeys);
        if (_key_resp1_1_allKeys.length > 0) {
          key_resp1_1.keys = _key_resp1_1_allKeys.map((key) => key.name);  // storing all keys
          key_resp1_1.rt = _key_resp1_1_allKeys.map((key) => key.rt);
          key_resp1_1.duration = _key_resp1_1_allKeys.map((key) => key.duration);
          // a response ends the routine
          continueRoutine = false;
        }
      }
...
function angrroutine1RoutineEnd(snapshot) {
    return async function () {
      //--- Ending Routine 'angrroutine1' ---
      angrroutine1Components.forEach( function(thisComponent) {
        if (typeof thisComponent.setAutoDraw === 'function') {
          thisComponent.setAutoDraw(false);
        }
      });
      psychoJS.experiment.addData('angrroutine1.stopped', globalClock.getTime());
      psychoJS.experiment.addData('textbox1a.text',textbox1a.text)
      // update the trial handler
      if (currentLoop instanceof MultiStairHandler) {
        currentLoop.addResponse(key_resp1_1.corr, level);
      }
      psychoJS.experiment.addData('key_resp1_1.keys', key_resp1_1.keys);
      if (typeof key_resp1_1.keys !== 'undefined') {  // we had a response
          psychoJS.experiment.addData('key_resp1_1.rt', key_resp1_1.rt);
          psychoJS.experiment.addData('key_resp1_1.duration', key_resp1_1.duration);
          routineTimer.reset();
          }
      
      key_resp1_1.stop();
      if (routineForceEnded) {
          routineTimer.reset();} else if (angrroutine1MaxDurationReached) {
          angrroutine1Clock.add(angrroutine1MaxDuration);
      } else {
          angrroutine1Clock.add(303.000000);
      }
      // Routines running outside a loop should always advance the datafile row
      if (currentLoop === psychoJS.experiment) {
        psychoJS.experiment.nextEntry(snapshot);
...
function question1RoutineBegin(snapshot) {
    return async function () {
      ...
      textbox_qu1.setText('');
      textbox_qu1.refresh();
      key_resp_qu1.keys = undefined;
      key_resp_qu1.rt = undefined;
      _key_resp_qu1_allKeys = [];
      psychoJS.experiment.addData('question1.started', globalClock.getTime());
      question1MaxDuration = null
      // keep track of which components have finished
      question1Components = [];
      question1Components.push(text_qu1);
      question1Components.push(textbox_qu1);
      question1Components.push(key_resp_qu1);
...
function question1RoutineEachFrame() {
    return async function () {
      ...
      // *text_qu1* updates
      if (t >= 0.0 && text_qu1.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        text_qu1.tStart = t;  // (not accounting for frame time here)
        text_qu1.frameNStart = frameN;  // exact frame index
        
        text_qu1.setAutoDraw(true);
      }
      
      
      // if text_qu1 is active this frame...
      if (text_qu1.status === PsychoJS.Status.STARTED) {
      }
      
      
      // *textbox_qu1* updates
      if (t >= 0.0 && textbox_qu1.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        textbox_qu1.tStart = t;  // (not accounting for frame time here)
        textbox_qu1.frameNStart = frameN;  // exact frame index
        
        textbox_qu1.setAutoDraw(true);
      }
      
      
      // if textbox_qu1 is active this frame...
      if (textbox_qu1.status === PsychoJS.Status.STARTED) {
      }
      
      
      // *key_resp_qu1* updates
      if (t >= 1 && key_resp_qu1.status === PsychoJS.Status.NOT_STARTED) {
        // keep track of start time/frame for later
        key_resp_qu1.tStart = t;  // (not accounting for frame time here)
        key_resp_qu1.frameNStart = frameN;  // exact frame index
        
        // keyboard checking is just starting
        psychoJS.window.callOnFlip(function() { key_resp_qu1.clock.reset(); });  // t=0 on next screen flip
        psychoJS.window.callOnFlip(function() { key_resp_qu1.start(); }); // start on screen flip
        psychoJS.window.callOnFlip(function() { key_resp_qu1.clearEvents(); });
      }
      
      // if key_resp_qu1 is active this frame...
      if (key_resp_qu1.status === PsychoJS.Status.STARTED) {
        let theseKeys = key_resp_qu1.getKeys({
          keyList: typeof 'space' === 'string' ? ['space'] : 'space', 
          waitRelease: false
        });
        _key_resp_qu1_allKeys = _key_resp_qu1_allKeys.concat(theseKeys);
        if (_key_resp_qu1_allKeys.length > 0) {
          key_resp_qu1.keys = _key_resp_qu1_allKeys[_key_resp_qu1_allKeys.length - 1].name;  // just the last key pressed
          key_resp_qu1.rt = _key_resp_qu1_allKeys[_key_resp_qu1_allKeys.length - 1].rt;
          key_resp_qu1.duration = _key_resp_qu1_allKeys[_key_resp_qu1_allKeys.length - 1].duration;
          // a response ends the routine
          continueRoutine = false;
        }
      }
...
function question1RoutineEnd(snapshot) {
    return async function () {
      ...
      psychoJS.experiment.addData('textbox_qu1.text',textbox_qu1.text)
      // update the trial handler
      if (currentLoop instanceof MultiStairHandler) {
        currentLoop.addResponse(key_resp_qu1.corr, level);
      }
      psychoJS.experiment.addData('key_resp_qu1.keys', key_resp_qu1.keys);
      if (typeof key_resp_qu1.keys !== 'undefined') {  // we had a response
          psychoJS.experiment.addData('key_resp_qu1.rt', key_resp_qu1.rt);
          psychoJS.experiment.addData('key_resp_qu1.duration', key_resp_qu1.duration);
          routineTimer.reset();
          }
